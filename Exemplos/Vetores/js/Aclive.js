...
function Trajetoria(nome, valor)
{
	if (ContTraj>0 && ContTraj<valor)		// 1301 é o número de objetos linhas para criar a trajetória (tentar alternativa que não baixe fps no futuro)
	{
		...
		lineGeometry.computeLineDistances();
		var lineMaterial = new THREE.LineBasicMaterial( { color: 'rgb(255,255,0)' } );
...
function UpdateEtiqueta(nome, mensagem)
{
	cena.remove(nome);
	Texto(mensagem);
}
